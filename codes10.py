...
import pandas as pd
# 读入数据
Titanic = pd.read_csv(r'F:\pycharm workspace\test\Titanic.csv')
Titanic.head()

# 删除无意义的变量，并检查剩余自字是否含有缺失值
Titanic.drop(['PassengerId','Name','Ticket','Cabin'], axis = 1, inplace = True) #1是列
...
Titanic.fillna(value = {'Age': Titanic.Age.mode()[0]}, inplace = True)
